...
Despite its simplicity, our system delivers competitive performance with a single MLLM call per fact-check at just \$0.03 on average using GPT5.1 via OpenAI batch API.
Our system is also easy to reproduce, consisting of only two straightforward retrievers and a prompt for evidence, label, and justification generation, which is why we suggest it as an accesible starting point for further experimentation.
