...
Despite its simplicity, our system delivers competitive performance with a single multimodal LLM call per fact-check at just \$0.03 on average using GPT5.1 via OpenAI batch API.
Our system is also easy to reproduce and tweak, consisting of only three decoupled modules -- a text-based retrieval module using similarity search, an image-based retrieval module based on API-accessed RIS, and a generation module using GPT5.1 -- which is why we suggest it as an accesible starting point for further experimentation.
