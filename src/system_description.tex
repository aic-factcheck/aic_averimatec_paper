...
\begin{enumerate}[label=\arabic*.]  % Second-level: 1., 2., 3.
    \item \textbf{Vector store} is produced for each of the \averitec{} datapoints in advance, using the scheme introduced in~\citealt{ullrich-etal-2024-aic}: the provided text-only \averitec{} knowledge store is chunked into 2048-character segments\footnote{The chunks do not overlap, and are annotated with context before and after in their metadata, as described in more detail in~\citealt{ullrich-etal-2024-aic}.}, and each is embedded using the \texttt{mxbai-embed-large-v1}~\cite{li-li-2024-aoe,emb2024mxbai} model.
