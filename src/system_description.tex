...
\end{enumerate}
\item \textbf{Image-based retrieval module} is invoked separately for each image attached to the \averitec{} claim -- that means, if claim contains $i$ images, $i$ separate sets of results will be produced
\begin{enumerate}[label=\arabic*.]  % Second-level: 1., 2., 3.
    ...
    Importantly, many results of RIS we performed in ii.1 were scraping-protected, most notably the Facebook and Instagram posts, resulting in an empty result. For simplicity and compliance with the intended data usage, we filter out these results as well, although we acknowledge that this might lead to a loss of useful information.
...
\begin{enumerate}[label=\arabic*.]  % Second-level: 1., 2., 3.
    ...
    \item Upon \textbf{parsing} the LLM outputs, we augment the LLM-generated evidence which refers an image-related source with a base64-encoded \textit{thumbnail} (ii.1) of the respective image to facilitatate comparison with evidence images chosen by human annotators. 
\end{enumerate}
\end{enumerate}
