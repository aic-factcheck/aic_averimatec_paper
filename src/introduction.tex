...
To facilitate the automation of this type of fact-checking~\citealt{cao2025averimatecdatasetautomaticverification} publishes \averitec{} dataset, collecting hundreds of reference image-text factchecks from human annotators, announcing the \averitec{} shared task to establish its state of the art.
...
With this paper, we introduce our 3rd place \averitec{} shared-task system which aims to give a strong baseline for image-text fact checking using easy-to-reproduce modules and affordable running costs, using a single query to multimodal LLM per claim and a single RIS request for each image attached to it.
